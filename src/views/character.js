...
class Character extends React.Component {
    ...
    bringTheHero = async (id) => {
        this.marvelService.getSelectedChar(id)
            .then(response => {
                this.setState({
                        ...
                        return (
                            <>
                                <div className="single-box" key="char.id">
                                    ...
                                    <img className="single-thumbnail" src={this.makeThumbnail(char.thumbnail.path, char.thumbnail.extension)} alt="{comic.title}"/>
                                    <h2 className="title-red bebas-neue">{char.name}</h2>
                                    <p>{this.makeDescription(char.description)}</p>
                                </div>
                                <div className="related-box">
                                    <h2 className="title-red bebas-neue">Comics relacionadas...</h2>
                                    <ul className="related-list">
                                        {char.comics.items.map((comic, index) => {
                                            return (
                                                <li className="related-item" key={index} onClick={() => this.props.history.push({pathname: `/comic/${this.getUrl(comic.resourceURI)}`, state:{charId: this.getUrl(comic.resourceURI)}})}>
                                                    {comic.name}
                                                </li>
                                            )
                                        })}
